...
export interface UseStepper<T extends string = string> {
  goToStep: (step: T, data?: unknown, force?: boolean) => void;
...
export const useStepper = <T extends string>(
  ...
  const goToStep = useCallback(
    (step: T, data?: unknown, force = false) => {
  ...
  const next = useCallback(() => {
    ...
    const nextStep = keys[keys.indexOf(step) + 1];
    if (nextStep) goToStep(nextStep);
  }, [step, goToStep, state]);

  const previous = useCallback(() => {
    const keys = Object.keys(state) as T[];
    const previousStep = keys[keys.indexOf(step) - 1];
    if (previousStep !== keys[keys.length - 1]) goToStep(previousStep as T);
  }, [step, goToStep, state]);
